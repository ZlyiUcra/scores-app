...
- [x] T001 Verify preconditions: `node --version` >= 22.12, `git status` clean, and re-run
...
- [x] T002 [US2] Edit `server/package.json`: `"cookie": "^0.6.0"` -> `"^0.7.2"` and
      `"express-rate-limit": "^7.4.0"` -> `"^8.0.0"`; leave `@types/cookie` at ^0.6.0 (compatible,
      no 0.7 types line exists - research.md R3)
- [x] T003 [US2] Run `npm install` in `server/` to regenerate `server/package-lock.json`; then
      verify per quickstart.md: `npm ls cookie` shows no 0.6.0 anywhere, `npm audit` reports 0
      vulnerabilities
- [x] T004 [US2] Run `npm run typecheck` and `npm run build` in `server/`; both must pass with
      zero source-code edits (limiters already use v8-compatible naming - research.md R3)
- [x] T005 [US2] Runtime smoke per quickstart.md "After commit 1": clean dev boot (no
...
- [x] T006 [US2] Report results to the user and request confirmation; after explicit "yes", create
...
- [x] T007 [P] [US1] Edit `client/package.json`: `"vite": "^5.4.2"` -> `"7.3.6"` (exact - it is
      also the rollback pin) and `"@vitejs/plugin-react": "^4.3.1"` -> `"^5.0.0"` (NOT ^6 - it
      requires vite 8; research.md R2)
- [x] T008 [P] [US1] Edit root `package.json`: `"engines": { "node": ">=22.5" }` ->
      `">=22.12"` (vite 7 engines requirement; constitution v1.0.1 already authorizes this)
- [x] T009 [US1] Run `npm install` in `client/` to regenerate `client/package-lock.json`; then
      verify per quickstart.md: `npm audit` reports 0 vulnerabilities and `npm ls launch-editor`
      is empty
- [x] T010 [US1] Confirm `client/vite.config.ts` requires NO edits (expected per research.md R5)
...
- [x] T011 [US1] Run `npm run typecheck` and `npm run build` in `client/`
- [x] T012 [US1] Dev-mode smoke per quickstart.md: `npm run dev` at repo root; login, change a
...
- [x] T013 [US1] Prod-served smoke per quickstart.md: build, start with `NODE_ENV=production` +
...
- [x] T014 [US1] Measure the built bundle vs baseline per quickstart.md (raw 350082 / gzip 109070
      for `client/dist/assets/index-*.js`); record the delta and weigh any regression before
      keeping the change
- [x] T015 [US1] Report results (including bundle delta) to the user and request confirmation;
...
- [x] T016 [US3] Verify the "Свідомо не робимо (backlog з тригерами повернення)" section of
